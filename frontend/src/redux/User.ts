...
const hasRole = (role: string, cognitoGroups: Array<string>) => {
    for (const group of cognitoGroups) {
        ...
        if (
            splitGroup.length > 1 &&
            splitGroup[splitGroup.length - 1] === role
        ) {
            return true;
        }
    }
...
export const userSlice = createSlice({
            ...
            prepare: (cognitoUser: any) => {
                return {
                    payload: {
                        isSignedIn: true,
                        organizationID:
                            cognitoUser.attributes["custom:OrganizationID"],
                        email: cognitoUser.attributes["email"],
                        name: cognitoUser.attributes["name"],
                        userName: cognitoUser["username"],
                        picture:
                            "picture" in cognitoUser.attributes
    ...
    extraReducers: (builder) => {
        builder.addCase(
            fetchOrganizationNameByID.fulfilled,
            (state, action) => {
...
export const selectGroupLeaderCognitoGroupName = (state: RootState) => {
    return (
        state.user.userState.organizationID + GROUPLEADER_COGNITOGROUP_SUFFIX
    );
};
