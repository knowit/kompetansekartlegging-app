...
stack =
  json[Object.keys(json).find((key) => key.startsWith('KompetanseStack-')) || 0]
// console.log(stack);
...
const functionMap = Object.keys(stack.functionMap).map((key) => {
  return {
    ...
    endpoint: stack.functionMap[key].endpoint.substring(
      0,
      stack.functionMap[key].endpoint.length - 1
